...
# @{Char::Eutf2::eS}        = qr{(?:(?:[\xC2-\xDF]|[\xE0-\xE0][\xA0-\xBF]|[\xE1-\xEC][\x80-\xBF]|[\xED-\xED][\x80-\x9F]|[\xEE-\xEF][\x80-\xBF]|[\xF0-\xF0][\x90-\xBF][\x80-\xBF]|[\xF1-\xF3][\x80-\xBF][\x80-\xBF]|[\xF4-\xF4][\x80-\x8F][\x80-\xBF])[\x00-\xFF]|[^\x80-\xFF\x09\x0A\x0B\x0C\x0D\x20])};
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Eutf2::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
# UTF-2 fold case lower case without parameter
#
sub Char::Eutf2::fc_() {
    my $s = $_;
    return join '', map {defined($fc{$_}) ? $fc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
{
    ...
    # UTF-2 mark last regexp matched
    sub Char::Eutf2::matched() {
        $last_s_matched = 0;
    }

    # UTF-2 mark last s/// matched
    sub Char::Eutf2::s_matched() {
        $last_s_matched = 1;
    }
    ...
    @Char::Eutf2::matched = (qr/(?{Char::Eutf2::matched})/);
}
...
sub charlist_not_qr {
    ...
    if (scalar(@multipleoctet) >= 1) {
        if (scalar(@singleoctet) >= 1) {
            ...
            return '(?!' . join('|', @multipleoctet) . ')(?:(?:[\xC2-\xDF]|[\xE0-\xE0][\xA0-\xBF]|[\xE1-\xEC][\x80-\xBF]|[\xED-\xED][\x80-\x9F]|[\xEE-\xEF][\x80-\xBF]|[\xF0-\xF0][\x90-\xBF][\x80-\xBF]|[\xF1-\xF3][\x80-\xBF][\x80-\xBF]|[\xF4-\xF4][\x80-\x8F][\x80-\xBF])[\x00-\xFF]|[^\x80-\xFF' . join('', @singleoctet) . '])';
        }
        else {
...
sub _parse_path {
    ...
    my @subpath = ();
    while ($path =~ /
        ((?: (?:[\xC2-\xDF]|[\xE0-\xE0][\xA0-\xBF]|[\xE1-\xEC][\x80-\xBF]|[\xED-\xED][\x80-\x9F]|[\xEE-\xEF][\x80-\xBF]|[\xF0-\xF0][\x90-\xBF][\x80-\xBF]|[\xF1-\xF3][\x80-\xBF][\x80-\xBF]|[\xF4-\xF4][\x80-\x8F][\x80-\xBF])[\x00-\xFF]|[^\x80-\xFF\/\\] )+?) [\/\\]
        /oxmsg
    ) {
        push @subpath, $1;
    }
...
=item Chop string
  ...
  character chopped. The Char::Eutf2::chop function is used primary to remove the newline
...
=item Reverse index string
  ...
  This function works just like Char::Eutf2::index except that it returns the byte
...
=item Lower case string
  ...
  This function returns a lowercased version of UTF-2 $string (or $_, if
  $string is omitted). This is the internal function implementing the \L escape
  in double-quoted strings.

  You can use the Char::Eutf2::fc function for case-insensitive comparisons via Char::UTF2
  software.
...
=item Lower case first character of string
  ...
  This function returns a version of UTF-2 $string with the first character
...
=item Upper case string
  ...
  This function returns an uppercased version of UTF-2 $string (or $_, if
...
=item Fold case string
  ...
  New to Char::UTF2 software, this function returns the full Unicode-like casefold of
  UTF-2 $string (or $_, if omitted). This is the internal function implementing
...
=item Make character
  ...
  ASCII or UTF-2, not Unicode. For the reverse of Char::Eutf2::chr, use Char::UTF2::ord.
...
=item Filename expansion (globbing)
  ...
  The Char::Eutf2::glob function grandfathers the use of whitespace to separate multiple

...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Char::Eutf2::function_ord     = 'Char::UTF2::ord'     if /\A ord \z/oxms;
                    $Char::Eutf2::function_ord_    = 'Char::UTF2::ord_'    if /\A ord \z/oxms;
                    $Char::Eutf2::function_reverse = 'Char::UTF2::reverse' if /\A reverse \z/oxms;
                    $Char::Eutf2::function_getc    = 'Char::UTF2::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Char::UTF2::escape();
...
=head1 Multiple-Octet Character Regular Expression (Char::UTF2 software provides)

Char::UTF2 software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Eutf2::ignorecase()' (Char::UTF2 software provides)

Char::UTF2 software applies calling 'Char::Eutf2::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Char::Eutf2::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Eutf2::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Char::Eutf2::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Char::Eutf2::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char::UTF2 software provides)

Char::UTF2 software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char::UTF2 software provide)
...
=item * Eval UTF-8 Script
  ...
  $eval = Char::UTF2::eval { block };
  $eval = Char::UTF2::eval $expr;
  $eval = Char::UTF2::eval;

  The Char::UTF2::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Char::UTF2::eval { block }
  and Char::UTF2::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Char::UTF2::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Char::UTF2::eval, the value returned from an Char::UTF2::eval is
  ...
  Char::UTF2::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Char::UTF2::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Char::UTF2::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Char::UTF2::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Char::UTF2::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Char::UTF2::eval cannot trap syntax errors.

  The Char::UTF2::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Char::UTF2::eval is complete, as do any subroutine or format definitions.
  The code of the Char::UTF2::eval is treated as a block, so any locally scoped
  variables declared within the Char::UTF2::eval last only until the Char::UTF2::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Char::UTF2::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Char::UTF2::eval { block }    eval { block }
  Char::UTF2::eval $expr        eval Char::UTF2::escape $expr
  Char::UTF2::eval              eval Char::UTF2::escape
  -------------------------------------------------

  To tell the truth, the subroutine Char::UTF2::eval does not exist. If it exists,
  you will troubled, when Char::UTF2::eval has a parameter that is single quoted
  string included my variables. Char::UTF2::escape is a subroutine that makes Perl
  ...
  Since Char::UTF2::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Char::UTF2::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Char::UTF2::eval $expr and Char::UTF2::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Char::UTF2::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Char::UTF2::eval { $answer = }; # WRONG

      # a runtime syntax error
      Char::UTF2::eval '$answer =';   # sets $@
  ...
  Using the Char::UTF2::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Char::UTF2::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Char::UTF2::eval { die "foo lives here" };
  ...
  With an Char::UTF2::eval, you should be especially careful to remember what's being
  looked at when:

      Char::UTF2::eval $x;        # CASE 1
      Char::UTF2::eval "$x";      # CASE 2

      Char::UTF2::eval '$x';      # CASE 3
      Char::UTF2::eval { $x };    # CASE 4

      Char::UTF2::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  a temporary is required if you want to mask some but not all errors:
      ...
      # alter $@ on nefarious repugnancy only
      {
          my $e;
          {
              ...
              Char::UTF2::eval { test_repugnancy() };
  ...
  The block of Char::UTF2::eval { block } does not count as a loop, so the loop
...
=head1 Un-Escaping bytes::* Subroutines (Char::UTF2 software provides)

Char::UTF2 software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Char::UTF2 software and

...
# @{Eutf2::eS}        = qr{(?:(?:[\xC2-\xDF]|[\xE0-\xE0][\xA0-\xBF]|[\xE1-\xEC][\x80-\xBF]|[\xED-\xED][\x80-\x9F]|[\xEE-\xEF][\x80-\xBF]|[\xF0-\xF0][\x90-\xBF][\x80-\xBF]|[\xF1-\xF3][\x80-\xBF][\x80-\xBF]|[\xF4-\xF4][\x80-\x8F][\x80-\xBF])[\x00-\xFF]|[^\x80-\xFF\x09\x0A\x0B\x0C\x0D\x20])};
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (/\A (?:$q_char)*? [*?] /oxms and (my @glob = Eutf2::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
{
    ...
    @Eutf2::matched = (qr/(?{Eutf2::matched})/);
}
...
This module is a run-time routines of the UTF2.pm.
Because the UTF2.pm automatically uses this module, you need not use directly.
...
=item Chop string
  ...
  character chopped. The Eutf2::chop function is used primary to remove the newline
...
=item Reverse index string
  ...
  This function works just like Eutf2::index except that it returns the byte
...
=item Lower case string
  ...
  You can use the Eutf2::fc function for case-insensitive comparisons via UTF2
  software.
...
=item Fold case string
  ...
  New to UTF2 software, this function returns the full Unicode-like casefold of
...
=item Filename expansion (globbing)
  ...
  The Eutf2::glob function grandfathers the use of whitespace to separate multiple

...
my $qq_substr  = qr{(?: UTF2::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_getc;        # getc    to getc    or UTF2::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'UTF2::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'UTF2::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'UTF2::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'UTF2::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $UTF2::re_t=%s; while($UTF2::re_t =~ %s){%s local $^W=0; local $UTF2::re_r=%s; %s$UTF2::re_t="$`$UTF2::re_r$'"; pos($UTF2::re_t)=length "$`$UTF2::re_r"; } return $UTF2::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$UTF2::re_r=eval $UTF2::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $UTF2::re_r=%s; %s"%s$UTF2::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $UTF2::re_n=0; while(%s =~ %s){%s local $^W=0; local $UTF2::re_r=%s; %s%s="$`$UTF2::re_r$'"; pos(%s)=length "$`$UTF2::re_r"; $UTF2::re_n++} return %s$UTF2::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $UTF2::re_r=%s; %s%s="%s$UTF2::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $UTF2::re_r=%s; %s%s="%s$UTF2::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$UTF2::re_r=eval $UTF2::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
By "use UTF2;", it automatically interpret your script as UTF-2. The various
...
=item * Returns Next Character
  ...
  If you import getc "use UTF2 qw(getc);", getc of your script will be rewritten in
  UTF2::getc. UTF2::getc is not compatible with getc of JPerl.
...
=item * Substr by UTF-2 Character
  ...
  If Perl version 5.14 or later, you can use the UTF2::substr() function as an
...
=item * Filename Globbing
  ...
  This operator is implemented via the Eutf2::glob() function. See Eutf2::glob of
  Eutf2.pm for details.

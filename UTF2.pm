...
impossible. UTF2 software is Perl program designed to make the "easy jobs easy".
...
=head1 Multiple-Octet Anchoring of Regular Expression (UTF2.pm provides)
...
UTF2.pm applies multiple-octet anchoring at beginning of regular expression.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Eutf2::anchor}(?:regexp).../
  --------------------------------------------------------------------------------
...
functions.
  ...
  index       Eutf2::index     Character
  rindex      Eutf2::rindex    Character
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Eutf2::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Eutf2::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Eutf2::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Eutf2::MATCH()       $1
  $MATCH          Eutf2::MATCH()       $1
  ${^MATCH}       Eutf2::MATCH()       $1
  $'              Eutf2::POSTMATCH()   $'
  $POSTMATCH      Eutf2::POSTMATCH()   $'
  ${^POSTMATCH}   Eutf2::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
